...
async def get_brand_for_imessage(imessage_id: str) -> Optional[dict]:
    ...
    try:
        ...
        all_brands = supabase.table("brand_agent") \
            .select("*") \
            .eq("is_active", True) \
            .execute()
        ...
        airstitch = next((b for b in matching_brands if "airstitch" in (b.get("brand_name", "") or "").lower()), None)
        if airstitch:
            print(f"✅ Found Airstitch brand: {airstitch.get('id')}")
            return airstitch
        
        # Otherwise return first match
        print(f"✅ Found brand: {matching_brands[0].get('brand_name')} (ID: {matching_brands[0].get('id')})")
